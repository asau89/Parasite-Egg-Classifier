...
All values are defined in `src/config.py` and can be changed there.
...
python -c "from src.model import get_model; from src.config import Config; m = get_model(Config()); import torch; print(m(torch.randn(1,3,384,384)).shape)"
