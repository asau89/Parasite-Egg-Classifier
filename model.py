"""
model.py — ConvNeXt model builder for parasite egg classification.

Uses timm (PyTorch Image Models) to load a pretrained convnext_base and
replaces the classification head with a 3-class linear layer.
"""
import torch
...
class ConvNeXtClassifier(nn.Module):
    """
    ConvNeXt backbone (pretrained on ImageNet-1K) with a custom 3-class head.

    Architecture:
        ConvNeXt backbone → LayerNorm → Dropout(0.3) → Linear(in_features, num_classes)

    The dropout before the final layer helps regularize the small dataset (300 train images).
    """
...
def get_model(config: Config) -> ConvNeXtClassifier:
    """
    ...
    Returns:
        ConvNeXtClassifier on the correct device
    """
    model = ConvNeXtClassifier(config)
