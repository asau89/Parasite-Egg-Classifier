"""
inference.py — Run predictions on unseen images using the trained ConvNeXt model.
...
Supports:
  ...
  Entire folder : python inference.py --folder path/to/folder/
...
def print_results_table(results: list[dict], classes: list[str]):
    col_file  = max(30, max(len(r["filename"]) for r in results) + 2)
    col_pred  = max(25, max(len(r["predicted_class"]) for r in results) + 2)

    print("\n" + "═" * (col_file + col_pred + 46))
    print("INFERENCE RESULTS")
    print("═" * (col_file + col_pred + 46))
    prob_headers = "  ".join(f"{cls[:15]:>15}" for cls in classes)
    print(f"{'Image':<{col_file}} {'Prediction':<{col_pred}} {'Confidence':>10}  {prob_headers}")
    print("─" * (col_file + col_pred + 46))
    for r in results:
        prob_vals = "  ".join(
        ...
        print(
            f"{r['filename']:<{col_file}} "
            f"{r['predicted_class'].replace('_', ' '):<{col_pred}} "
            f"{r['confidence']:>9.2f}%  "
            f"{prob_vals}"
        )
    print("═" * (col_file + col_pred + 46))
...
def save_grid(results: list[dict], classes: list[str], out_path: Path,
    ...
    for idx, result in enumerate(results):
        ...
        label_text = f"{pred_cls.replace('_', ' ')}\n{confidence:.1f}%"
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_xlabel(result["filename"], fontsize=6, color="#555555")
    ...
    # Legend
    legend_patches = [
        mpatches.Patch(color=color, label=cls.replace("_", " "))
...
def main():
    parser = argparse.ArgumentParser(
        description="Run ConvNeXt parasite classifier on unseen images",
    ...
    parser.add_argument("--folder", metavar="DIR",
                        help="Folder of images to classify")
