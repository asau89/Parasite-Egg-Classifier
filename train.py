"""
train.py — Full GPU-accelerated training loop for ConvNeXt parasite classifier.

Usage:
  python train.py                        # Single training run (seed from config)
  python train.py --seeds 42,123,456,789,1234  # 5-seed multi-run (mean ± std)
  python train.py --debug                # CPU smoke test: 1 epoch, 2 batches
...
from utils import EarlyStopping, FocalLoss, save_history, plot_training_history, set_seed
...
def main():
    parser = argparse.ArgumentParser(description="Train ConvNeXt parasite classifier")
    ...
    if len(seeds) > 1:
        ...
        print(f"{'─'*60}")
        print(f"{'Mean':}{' '*4} {mean_acc:>9.2f}%  ±{std_acc:.2f}%")
