"""
tune.py — Phased hyperparameter grid search for ConvNeXt parasite classifier.
...
Usage:
  python tune.py --phase 1          # Learning Rate sweep
  python tune.py --phase 2          # Batch Size + Weight Decay
  python tune.py --phase 3          # Dropout + Label Smoothing
  python tune.py --phase 4          # Model Variant + Resolution
  python tune.py --phase 1 --dry-run  # Print trial configs, no training
...
def build_phase_grid(phase: int, best: dict) -> list[dict]:
    ...
    # ── Shared base: all locked-in best values ────────────────────────────────
    base = dict(
    ...
    if phase == 1:
        ...
        # Model Variant + Resolution (optional)
        return [
            {**base, "model_name": "convnext_tiny",  "image_size": 224, "batch_size": 32},
            {**base, "model_name": "convnext_small", "image_size": 384, "batch_size": 16},
            {**base, "model_name": "convnext_base",  "image_size": 384, "batch_size": 16},  # baseline
        ]
    elif phase == 5:
        ...
        return [
            {**base, "backbone_lr_multiplier": 0.1},    # very small backbone LR
            {**base, "backbone_lr_multiplier": 0.3},
            {**base, "backbone_lr_multiplier": 1.0},    # baseline (same LR for all)
        ]
    elif phase == 8:
...
def main():
    parser = argparse.ArgumentParser(description="Phased hyperparameter tuning for ConvNeXt")
