...
def visualize_samples(cfg, checkpoint_path, image_paths, output_dir="outputs/cams"):
    ...
    # Find target layer (last layer of last stage of ConvNeXt backbone)
    # For timm convnext, it is usually model.backbone.stages[-1].blocks[-1]
    # We can try to find it dynamically
    target_layer = None
    ...
    for img_path_str in image_paths:
        ...
        # Note: input size is cfg.image_size
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_file", type=str, default="test_set.txt", help="File containing image names (default: test_set.txt)")
    parser.add_argument("--limit", type=int, default=None, help="Limit number of images to process")
    args = parser.parse_args()

    cfg = Config()
    # Default device is handled by Config.__post_init__()
    ...
    if args.images:
        ...
        with open(args.dataset_file, "r") as f:
            paths = [line.strip() + ".jpg" for line in f.readlines() if line.strip()]
